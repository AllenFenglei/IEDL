...
def build_AOLDA_input_version(timed_reviews=None):
    ...
    for apk, reviews in timed_reviews.items():
        ...
        dictionary.filter_extremes(no_below=5, keep_n=10000)
        dictionary.compactify()

        # for each interval, build bow
        input_X = []
        temp_X = []
        #f = open('../dataset/test_views.txt', 'r')
        #paramter = []
        #for i in f:
        #    if(int(i)<10):
        #        paramter.append(int(1))
        #    elif(int(i)<20):
        #        paramter.append(int(2))
        #    else:
        #        paramter.append(int(3))
        #f.close()
        for t_i, text_period in enumerate(input):
            ...
            r_id = 0
            #print(temp_X)
            for k, text in enumerate(temp_X):
                empty = True
            ...
            #print(len(row))
            #print(value)
            input_X.append(sparse.coo_matrix((value, (row, col)), shape=(len(temp_X), len(dictionary))))#row n, col m stores value, otherwise 0
        print(len(input_X))
...
def get_candidate_sentences_ids(rawinput, rates):
    ...
    #print(sent_rates)
    print(len(sent_rates))  #3
    print(len(sent_rates[0]))   #1458
    print(len(sent_rates[1]))   #2164
    print(len(sent_rates[2]))   #1742
    return sent_ids, sent_rates

def get_sensitivities(dic, rawinput, rates, label_ids):
    sensi = []
    for t_i, rawinput_i in enumerate(rawinput): #3 loops since 3 versions
        sensi_t = []
        ...
        for rl in label_sensi:
            ...
            #sensi_t.append(np.exp(- m_rl[0]))
        sensi.append(np.array(sensi_t))
    #print(sensi)
    #print(len(sensi))
    #print(sensi[0].shape)   #(57,)
    #print(sensi[1].shape)   #(89,)
    #print(sensi[2].shape)   #(67,)
    return sensi
