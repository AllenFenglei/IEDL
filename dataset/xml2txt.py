...
def extract(filename):
	...
	root = tree.getroot()
	out = codecs.open(filename+'.txt', 'w', 'utf-8')
	count = 0
		...
		if X != "":
			...
			if title !=  None:
				...
				if ParentID != None:
					...
					out_title = "None"
			if view_count != None:
				ID_views[ID] = view_count
				out_views = view_count
			else:
				if ParentID != None:
					out_views = ID_views[ParentID]
				else:
					out_views = "355"	#the average

			
		
			if count < 100000 and X != " ":
				X = str(score) + "******" + X + "******" + out_title.lower() + "******" + out_time + "******"+ str(version) + "******" + str(out_views)
				out.write(X+'\n')
				count = count + 1
